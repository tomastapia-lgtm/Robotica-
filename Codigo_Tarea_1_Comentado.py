# Importación de librerías necesarias para el control del dron, la identificacion de gestos, medicion del tiempo y apriltags
...
class GestureDetector:
    def __init__(self, model_path='gesture_recognizer.task'):
        ...
        options = vision.GestureRecognizerOptions(
            ...
            min_hand_detection_confidence=0.5, # Qué tan estricta es el modelo para reconocer una mano nueva (valores bajos = reconoce más fácil, pero puede equivocarse).
            min_hand_presence_confidence=0.5,  # Qué tan estricta es el modelo para no perder de vista la mano que ya estaba siguiendo.
            min_tracking_confidence=0.5,       # Precisión exigida para rastrear el movimiento de los dedos sin que el dron se confunda.
...
detector_gestos = GestureDetector()           # Instanciamiento de los modelos de visión artificial
detector_tags = Detector(families="tag36h11") # Se configura el modelo de AprilTags

tiempo_limite_tags = 0  # Temporizador límite para mantenerse dentro del modo de seguimiento autónomo
...
#        PARÁMETROS DE SEGUIMIENTO (TRACKING)
# Variables de control Proporcional para mantener al dron estabilizado frente a un objetivo visual
TARGET_SIZE = 120  # Distancia al objetivo: Define qué tan cerca o lejos debe mantenerse el dron.
KP_YAW = 0.3       # Sensibilidad de giro: Qué tan rápido rota el dron hacia la izquierda o derecha.
KP_UD = 0.4        # Sensibilidad de altura: Qué tan rápido sube o baja el dron.
KP_FB = 0.4        # Sensibilidad de avance: Qué tan rápido vuela hacia adelante o hacia atrás.
...
while True:
    ...
    # frame+=10
    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) # Corrección de canal de colores 
    ...
    if modo_tags_activo:
        ...
        # El detector de tags necesita trabajar sobre una imagen en escala de grises
        ...
        if len(resultado_tags) > 0:
            ...
            # 1. Determinar las coordenadas centrales del Tag y evaluar su dimensión aparente mediante el promedio de su anchura y altura
            ...
            # 2. Obtener las métricas de error: la diferencia exacta en píxeles entre el centro visual de la cámara y la posición/escala del Tag
            error_x = cx - centro_cam_x
            error_y = cy - centro_cam_y
            error_size = TARGET_SIZE - size_px

            # 3. Generar las velocidades de control proporcional (multiplicando el margen de error por su respectiva ganancia y restringiendo el rango de acción a 50)
            yaw = int(np.clip(error_x * KP_YAW, -50, 50))
            ud = int(np.clip(-error_y * KP_UD, -50, 50)) 
            fb = int(np.clip(error_size * KP_FB, -50, 50))

            # Zonas muertas: Si el dron está muy cerca del centro deseado, asigna velocidad cero para evitar temblores excesivos
        ...
        if not apriltags_detectado:
            # Si se perdió de vista el Tag en este frame, frena por completo el dron
            lr, fb, ud, yaw = 0, 0, 0, 0
        ...
        # Delegla detección de la mano al modelo usado 'gesture_recognizer.task'
    ...
    # Permite al desarrollador intervenir manualmente forzando velocidades con comandos en consola
    if USE_PYNPUT:
        cv2.pollKey()
        pressed = keys.copy()
    else:   
    ...
    if 'w' in pressed: fb = speed_manual     # 'w' es la tecla de avance
    if 's' in pressed: fb = -speed_manual    # 's' es la tecla de retroceso
    if 'a' in pressed: lr = -speed_manual    # 'a' es la tecla de izquierda
    if 'd' in pressed: lr = speed_manual     # 'd' es la tecla de derecha
    if 'r' in pressed: ud = speed_manual     # 'r' es la tecla de vuelta
    if 'f' in pressed: ud = -speed_manual    # 'f' es la tecla de frente
    if 'q' in pressed: yaw = -speed_manual   # 'q' es la tecla de giro a la izquierda
    if 'e' in pressed: yaw = speed_manual    # 'e' es la tecla de giro a la derecha
    ...
    #Dibujar recuadro de lectura cruda y estado actual de acción en la esquina inferior izquierda
